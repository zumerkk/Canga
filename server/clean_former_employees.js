...
const csvContent = fs.readFileSync(path.join(__dirname, '..', 'İŞTEN AYRILANLAR-Tablo 1.csv'), 'utf8');
...
(async () => {
  ...
  for (const csvEmp of formerEmployees) {
    ...
    if (existing) {
      // Mevcut kaydı güncelle - sadece temel bilgileri güncelle
      const updateData = {
      ...
      // Tarih alanlarını sadece geçerliyse güncelle
      const dogumDate = parseDateDMY(csvEmp.dogumTarihi);
      if (dogumDate) updateData.dogumTarihi = dogumDate;
      
      const giriDate = parseDateDMY(csvEmp.iseGirisTarihi);
      if (giriDate) updateData.iseGirisTarihi = giriDate;
      
      await Employee.findByIdAndUpdate(existing._id, updateData, { runValidators: false });
      updatedCount++;
    } else {
      // Yeni kayıt ekle - departman/pozisyon ÜRETİM olarak ayarla
      ...
      const newEmp = new Employee({
        ...
        durum: 'AYRILDI',
        ayrilmaSebebi: 'İşten ayrılma',
        departman: 'ÜRETİM',  // Varsayılan departman
        pozisyon: 'İMAL İŞÇİSİ',  // Varsayılan pozisyon
        lokasyon: 'MERKEZ'
      });
