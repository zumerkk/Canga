...
const ROUTE_NAME = 'OSMANGAZİ - ÇARŞI MERKEZ SERVİS GÜZERGAHI';
const CSV_PATH = path.join(__dirname, '..', '..', 'PERSONEL SERVİS DURAK ÇİZELGESİ 22.09', 'OSM ÇARŞI MRK-Tablo 1.csv');
...
function parseStops(rows){
  ...
  for (const row of rows){
    ...
    if (!inPassengers){
      if (first && !/SERVİS GÜZERGAHI/i.test(first) && !/HAREKET SAATİ/i.test(first) && !/OSMANGAZİ-ÇARŞI MERKEZ/i.test(first)){
        stops.push(first);
      }
    }
  }
...
async function main(){
  console.log('🚀 OSMANGAZİ route stop updater starting...');
