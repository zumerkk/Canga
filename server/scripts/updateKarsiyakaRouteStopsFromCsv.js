...
const ROUTE_NAME = 'BAHÇELİEVLER-KARŞIYAKA MAHALLESİ SERVİS GÜZERGAHI';
const CSV_PATH = path.join(__dirname, '..', '..', 'PERSONEL SERVİS DURAK ÇİZELGESİ 22.09', 'KARŞIYAKA-Tablo 1.csv');
...
function parseStops(rows){
  ...
  for (const row of rows){
    ...
    if (!inPassengers){
      if (first && !/SERVİS GÜZERGAHI/i.test(first) && !/HAREKET SAATİ/i.test(first) && !/BAHÇELİEVLER-KARŞIYAKA/i.test(first)){
        stops.push(first);
      }
    }
  }
...
async function main(){
  console.log('🚀 KARŞIYAKA route stop updater starting...');
